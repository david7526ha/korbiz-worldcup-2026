...
// ═══ 32강 진출 확정 판정 - 단일 통합 로직 (캐시 적용, 화면 렌더링당 1회만 전체 계산) ═══
// 원칙: "이 팀이 32강에 못 갈 수 있는 시나리오가 단 하나라도 존재하는가?"
// 성능: matchResults 내용이 같으면 캐시 재사용. margins는 0/2/5 3단계로 축소(승점 추월 여부 판정엔 충분).

var __clinchCache = { key: null, cutoff: null, perGroup: {} };
...
function generateGroupScenarios(group, matchResults) {
  ...
  var margins = [0, 2, 5]; // 3단계로 축소 (18->9 combos per match, 전수탐색 의미는 동일하게 유지)
  var scenarios = [];
...
// matchResults 내용을 캐시 키로 사용해서, 동일한 데이터면 절대 재계산하지 않음
function getClinchCache(tournament) {
  var mr = tournament.matchResults || {};
  var key = JSON.stringify(mr) + "|" + JSON.stringify(tournament.groupResults || {});
  if(__clinchCache.key === key) return __clinchCache;

  // 새 데이터 -> 전체 12개 조 시나리오 + 컷오프를 딱 한 번만 계산
  ...
  Object.keys(GROUPS).forEach(function(group) {
    ...
    var scenarios2 = generateGroupScenarios(group, mr);
    perGroup[group] = scenarios2;
    var best = null;
  ...
  var cutoff = thirds[7] || { pts: -1, gd: -99, gf: -99 };

  __clinchCache = { key: key, cutoff: cutoff, perGroup: perGroup };
  return __clinchCache;
}
...
function isClinched(team, group, tournament) {
  ...
  var cache = getClinchCache(tournament);
  var scenarios = cache.perGroup[group] || [];
  var cutoff = cache.cutoff;
