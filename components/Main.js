...
// ═══ 32강 진출 확정 판정 - 서버에서 미리 계산된 clinchStatus를 읽기만 함 ═══════════
// 무거운 연산은 /api/compute-clinch (서버사이드)에서 한 번만 수행하고 Firestore에 저장.
// 클라이언트는 그 값을 단순 조회만 하므로 브라우저 멈춤 위험이 전혀 없음.
function estimateAdvanceTo32(team, group, tournament) {
  ...
  var clinchStatus = tournament.clinchStatus || {};
  if(team in clinchStatus) return clinchStatus[team];
  return 0.5; // 서버 계산 전이면 중립값
}
function isTeamEliminated(team, group, tournament) {
  return estimateAdvanceTo32(team, group, tournament) === 0.02;
}
...
function AdminPanel({tournament,users,onClose,showToast,t,lang}){
  ...
  const save=async()=>{
    ...
    try{
      ...
      // 32강 확정 상태를 서버에서 재계산 (실패해도 저장 자체엔 영향 없음)
      fetch('/api/compute-clinch').catch(function(){});
      showToast("Saved ✓");
      onClose();
